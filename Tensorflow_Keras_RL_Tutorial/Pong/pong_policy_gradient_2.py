...
def main():
    ...
    # hyperparameters
    batch_size = 10 
    # how many rounds we play before updating the weights of our network
    gamma = 0.99 # discount factor for reward
    decay_rate = 0.99 # Parameter used in RMSProp algorithm
    num_hidden_layer_neurons = 200
    input_dimensions = 80 * 80
    learning_rate = 1e-3 
    # learning_rate = The rate at which we learn from our results 
    # to compute the new weights. A higher rate means we react more 
    # to results and a lower rate means we don’t react as strongly 
    # to each result.
    ...
    # Layer 1 of our Neural Network is a 200 x 6400 matrix representing
    # the weights for our hidden layer. 
    # For layer 1, element w1_ij represents 
    # the weight of neuron i for input pixel j in layer 1.
    # Layer 2 is a 200 x 1 matrix representing the weights of the output
    # of the hidden layer on our final output. 
    # For layer 2, element w2_i represents the weights we place on the 
    # activation of neuron i in the hidden layer.
    # The weights are divided by sqrt() according to Xavier Normalization
    weights = {
        'W1': np.random.randn(num_hidden_layer_neurons, input_dimensions) / np.sqrt(input_dimensions),
        'W2': np.random.randn(num_hidden_layer_neurons) / np.sqrt(num_hidden_layer_neurons)
    }
    ...
    while True:
        ...
        # probability of telling our AI to move up
        hidden_layer_values, up_probability = \
        apply_neural_nets(processed_observations, weights)
    
        episode_observations.append(processed_observations)
        episode_hidden_layer_values.append(hidden_layer_values)

        # We choose an action by flipping an imaginary coin that lands “up”
        # with probability up_probability and down with 1 - up_probability. 
        # If it lands up, we choose tell our AI to go up and 
        # if not, we tell it to go down
        ...
        if done: # an episode finished
            ...
            # When we notice we are done, the first thing we do is 
            # compile all our observations and gradient calculations for
            # the episode. This allows us to apply our learnings over all 
            # the actions in the episode.
            # Combine the following values for the episode
            episode_hidden_layer_values = \
            np.vstack(episode_hidden_layer_values)
            episode_observations = np.vstack(episode_observations)
            episode_gradient_log_ps = np.vstack(episode_gradient_log_ps)
            episode_rewards = np.vstack(episode_rewards)

            # Next, we want to learn in such a way that actions taken 
            # towards the end of an episode more heavily influence our 
            # learning than actions taken at the beginning. This is called 
            # discounting.

            # Think about it this way - if you moved up at the first frame 
            # of the episode, it probably had very little impact on 
            # whether or not you win. However, closer to the end of the 
            # episode, your actions probably have a much larger effect as 
            # they determine whether or not your paddle reaches the ball 
            # and how your paddle hits the ball.

            # We’re going to take this weighting into account by 
            #discounting our rewards such that rewards from earlier frames 
            # are discounted a lot more than rewards for later frames. 
            # After this, we’re going to finally use backpropagation to 
            # compute the gradient (i.e. the direction we need to move our 
            # weights to improve).
            # Tweak the gradient of the log_ps based on the discounted 
            # rewards
            episode_gradient_log_ps_discounted = \
            discount_with_rewards(episode_gradient_log_ps, \
                                  episode_rewards, gamma)
            ...
            if episode_number % batch_size == 0:
                update_weights(weights, expectation_g_squared, g_dict,\
...
def preprocess_observations(input_observation, prev_processed_observation, input_dimensions):
    ...
    # select alternate pixels from rows and columns
...
def discount_with_rewards(gradient_log_p, episode_rewards, gamma):
    ...
    # standardize the rewards to be unit normal (helps control the 
